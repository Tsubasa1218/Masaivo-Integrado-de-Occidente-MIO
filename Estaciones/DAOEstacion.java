...
public class DAOEstacion {
    ...
    public void agregarEstaciones(javax.swing.JComboBox combo){
    ...
    public int consultarEstacion(String nombreEst){
        int estacion = 0;
        try { 
            String consultaEstActual = "SELECT id_estacion FROM estaciones WHERE nombre_estacion  = '" + nombreEst + "';";
            Connection conn= fachada.getConnetion();
            Statement sentencia = conn.createStatement();
            ResultSet tabla = sentencia.executeQuery(consultaEstActual);
            while (tabla.next()){
                estacion = tabla.getInt("id_estacion");
            }
            tabla.close();
    ...
    public String consultarEstacionSolicitud(String tiquete){
        String estacion = "";
        try { 
            String consultaEstActual = "SELECT nombre_estacion FROM estaciones WHERE id_estacion IN"
                + "(SELECT sol.id_estacion FROM solicitudes_presentadas sol WHERE sol.tiquete = '" + tiquete + "');";
            Connection conn= fachada.getConnetion();
            Statement sentencia = conn.createStatement();
            ResultSet tabla = sentencia.executeQuery(consultaEstActual);
            while (tabla.next()){
                estacion = tabla.getString("nombre_estacion");
            }
            tabla.close();
            return estacion;
        } catch (SQLException e) {
